// src/app/studashboard/marketplace/business/[id]/products/[productId]/edit/page.tsx
//
// "Update Inventory" — the product edit form, reached from a product card
// on the Products tab. Fetches the product's full detail, then submits
// changes via PATCH /api/marketplace/businesses/[id]/products/[productId].
...
export default function EditProductPage({ params }: { params: Promise<{ id: string; productId: string }> }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await api.get<{ product: ProductDetail }>(`/marketplace/businesses/${id}/products/${productId}`);
        if (cancelled) return;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api.patch(`/marketplace/businesses/${id}/products/${productId}`, form);
