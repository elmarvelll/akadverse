...
import { AlertCircle, ExternalLink, Loader2, UserPlus, X } from "lucide-react";
        ...
        <div key={slot.id} className="flex flex-wrap items-center gap-3 bg-white rounded-2xl border border-gray-100 p-4">
          ...
          <label className="flex items-center gap-1.5 text-xs text-gray-500">
            ...
            <input
              type="number"
              min={0}
              defaultValue={slot.delivererCapacity}
              disabled={savingId === slot.id}
              onBlur={(e) => {
                const value = Math.max(0, Math.trunc(Number(e.target.value)));
                if (value !== slot.delivererCapacity) save(slot, { delivererCapacity: value });
              }}
          ...
          <label className="flex items-center gap-1.5 text-xs text-gray-500">
            ...
            <input
              type="number"
              min={0}
              defaultValue={slot.bookingCutoffMins}
              disabled={savingId === slot.id}
              onBlur={(e) => {
                const value = Math.max(0, Math.trunc(Number(e.target.value)));
                if (value !== slot.bookingCutoffMins) save(slot, { bookingCutoffMins: value });
              }}
              className="w-16 px-2 py-1 border border-gray-200 rounded-lg text-xs"
            />
          </label>
          <label className="flex items-center gap-1.5 text-xs text-gray-500">
            ...
            <input
              type="number"
              min={0}
              defaultValue={slot.vendorPrepDeadlineMins}
              disabled={savingId === slot.id}
              onBlur={(e) => {
                const value = Math.max(0, Math.trunc(Number(e.target.value)));
                if (value !== slot.vendorPrepDeadlineMins) save(slot, { vendorPrepDeadlineMins: value });
              }}
              className="w-16 px-2 py-1 border border-gray-200 rounded-lg text-xs"
            />
          </label>
          <button
            ...
            className={`ml-auto text-xs font-medium px-3 py-1.5 rounded-full transition ${
