...
// `role` (for role-based home-route redirects, see src/proxy.ts),
// `firstName` (so the dashboard can greet the user without a second fetch —
// see src/context/AuthContext.tsx), and `isAdmin` (Marketplace admin
// access, independent of `role` — see prisma/schema.prisma's comment on
// User.isAdmin and src/lib/admin.ts#requireAdmin). This file uses TypeScript's
